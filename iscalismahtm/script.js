...
cn.Open(strConn); 
	...
	function ekle(month,miktar) {
			if (config.data.datasets.length > 0) {
				//var month = MONTHS[config.data.labels.length % MONTHS.length];
				config.data.labels.push(month.toString());

				config.data.datasets.forEach(function(dataset) {
					dataset.data.push(miktar);
				});
...
var SQL = "select m.cMiktar as mik,c.cAd as ad,c.birim as birim,  m.fiyat as fiyat,(select bTarih from isi where iNo=m.iNo) " +
...
rs.Close(); 
	...
	rs = new ActiveXObject("ADODB.Recordset");
SQL = "select iNo from isi where ad='"+sel+"' order by iNo asc"; 
rs.Open(SQL, cn);
 while (!rs.eof){
...
else if(fiyat.toString().length==0)alert("Lütfen gecerli bir fiyat giriniz!");
else{ 
// Code to connect the MS Access database using java Script
// " C:/my_db.mdb " is the MS Access database
rs = new ActiveXObject("ADODB.Command");
// Assume there is a table in MS Access database with the name " user_master “, below is the query for authentication 
rs.ActiveConnection = cn;
rs.CommandText = "INSERT INTO miktar(iNo,cMiktar,cNo,fiyat)VALUES("+id+","+adet+","+calisma+","+fiyat+")";
if(rs.Execute()){

 //rs.Close();
 rs = new ActiveXObject("ADODB.Recordset");
// Assume there is a table in MS Access database with the name " user_master “, below is the query for authentication
var SQL = "select ID,(select cAd from calisma where cNo=miktar.cNo) as a,(select birim from calisma where cNo=miktar.cNo) as b,fiyat,cMiktar from miktar where iNo in( select max(iNo) from isi)";  
rs.Open(SQL, cn);
document.getElementById("table").innerHTML=' ';
